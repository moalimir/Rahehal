ALTER TABLE step_up_attempt
  ...
  CHECK (
    ...
    OR correlation_id ~ '^cor_[A-Za-z0-9][A-Za-z0-9_-]{2,63}$'
  );
...
BEGIN
  IF NEW.status <> 'pending'
    ...
    RAISE EXCEPTION 'step-up attempt requires correlation, the active session and exact challenge scope'
      USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
...
BEGIN
  ...
  IF target.id IS NULL
     ...
     OR (TG_OP = 'INSERT' AND (
       ...
       OR NEW.expires_at <> 'infinity'::timestamptz
       OR NOT EXISTS (
