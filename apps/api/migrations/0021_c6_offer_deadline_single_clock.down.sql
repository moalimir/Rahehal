-- Restores 0018's `protect_c6_direct_offer`, including the second clock this
-- migration removed. Reversing it reinstates the behaviour where a send that
-- the application clock authorised is refused by the database clock, so a
-- rollback should be paired with a rollback of the application build.
