...
The current repository is an advanced frontend prototype plus a locally authoritative challenge-publication slice, not a production system. Phase 1 supplies PostgreSQL identity/workspace/session boundaries, immutable challenge drafts, local OIDC, and a same-origin connected browser. Phase 2 carries that slice through rough-brief ops triage, full formulation readiness, version-specific distinct-actor approval gates and rejected-version rework, atomic publication, a structurally separate public projection, live-call controls, and a purpose/gate-scoped platform queue/brief. PostgreSQL migration `0012` closes the Phase-2 review gaps, including projection insert synchronization and approvals-time eligibility snapshots. The web still builds in two modes — `RAHHAL_WEB_RUNTIME=network` is the connected client, and the default static export remains a non-authoritative demo. The worker remains demo-only. A managed production OIDC provider, MFA/step-up, RLS, durable worker claims, independently operated audit/WORM export, private storage, and external provider integrations are not implemented. Current status and remaining gates are owned by `project-documents/82_PHASE0_COMPLETION.md`; delivery order is owned by `project-documents/80_DELIVERY_ROADMAP.md`.
