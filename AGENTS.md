...
The current repository is an advanced frontend prototype plus an initial API/worker walking skeleton, not a production system. A1a supplies the PostgreSQL foundation, A1b supplies session/workspace authorization and a transaction-scoped unit of work, A1c supplies an explicitly composed PostgreSQL challenge-draft create/read/save path with immutable versions and atomic receipt/audit/outbox/idempotency evidence, A2 supplies a provider-neutral authorization-code + PKCE boundary against a synthetic local Dex identity provider, and A3 wires the browser to that API through same-origin HttpOnly session cookies, so the challenge-draft slice is server-authoritative end to end. The web still builds in two modes — `RAHHAL_WEB_RUNTIME=network` is the connected client, and the default static export remains a non-authoritative demo. The worker remains demo-only. A managed production OIDC provider, MFA/step-up, RLS, durable worker claims, independently operated audit/WORM export, private storage, and external provider integrations are not implemented. The delivery strategy is **depth before breadth**: make one thin vertical slice authoritative before adding more screens. Current status and remaining gates are owned by `project-documents/82_PHASE0_COMPLETION.md`; delivery order is owned by `project-documents/80_DELIVERY_ROADMAP.md`.
