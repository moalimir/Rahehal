...
**M1 implementation update (2026-09-16):** owner-controlled authoring/publication is now implemented and locally verified; M2 matching is still a target. Active owners can publish complete unpublished versions and manage live calls in their own organization. Other org roles retain their delegated authoring/approval/publishing permissions. Migration `0022` adds immutable owner-publication attribution and preserves existing gate evidence; the current schema has 21 migrations. See `project-documents/86_M1_OWNER_PUBLICATION.md` for tests and the human release-review gate. Older references below to the restored Phase 3 baseline or awaiting M1 describe the pre-M1 checkpoint, not current executable owner policy.

**Active owner decision (2026-09-16):** DEC-2026-018 in `project-documents/25_DECISIONS.md` and `project-documents/26_LEAN_MVP_SCOPE.md` define the lean MVP: restored Phase 3 (`3f80192`), owner-controlled challenge publication, independent organization–solver interactions, and a minimal final selection/match. The former Phase 4 programme is deferred and archived; do not resume its reviewer/platform workflows as MVP requirements. Owner publication and minimal matching are target changes, not implemented baseline capabilities. Final matching requires organization selection and explicit solver acceptance, without routine platform approval; record a shared agreement summary and support rejection, decline, withdrawal, cancellation and no-award. Integrated payments are deferred in favor of off-platform settlement. Detailed timing/role rules remain to be specified; external-user readiness requires real authentication, one outbound notification channel and a decision on whether secure private files are necessary. Preserve the canonical vocabulary and server/data safeguards; the owner-approved exception to mandatory publication separation is recorded in DEC-2026-018.
...
The current repository is an advanced frontend prototype plus locally authoritative Phases 1–3, not a production system. Phase 1 supplies PostgreSQL identity/workspace/session boundaries, immutable challenge drafts, local OIDC, and a connected browser. Phase 2 supplies the existing governed publication path, public projections, and live-call controls; those baseline gates remain until M1 implements the accepted owner-publication policy. Phase 3 supplies development-provider solver activation, individual/team workspaces, versioned eligibility, immutable proposals and revisions, clarification, direct offers, and durable PostgreSQL notification projection. The restored baseline has 20 migrations through `0021`. `RAHHAL_WEB_RUNTIME=network` is connected; the default static export is a non-authoritative demo. Managed production identity/contact delivery and MFA, RLS, independently operated audit/WORM export, private storage, and external integrations remain release gaps. Current status is owned by `project-documents/82_PHASE0_COMPLETION.md`; active scope by `project-documents/26_LEAN_MVP_SCOPE.md`; delivery order by `project-documents/80_DELIVERY_ROADMAP.md`.
...
8. **Attribution and scoped authority are real.** DEC-2026-018 authorizes routine owner-controlled publication for the lean MVP; independent platform/technical/legal/finance publication approvals are not target MVP prerequisites. Published evidence remains version-specific and attributable, and the baseline policy is not changed until M1 is implemented. Deferred formal reviewer materials still require a clear first-class COI declaration. Technical acceptance, finance approval, and effective contract remain separate gates if payments are later enabled. Provider callbacks are authenticated, replay-safe, and reconciled, never sole authority.
...
| Active lean MVP scope and remaining match policies | `project-documents/26_LEAN_MVP_SCOPE.md`                                                            |
