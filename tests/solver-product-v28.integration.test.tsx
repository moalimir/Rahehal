...
describe("محصول canonical فرد و تیم نسخه ۲۸", () => {
  it("drawer موبایل Escape، aria-expanded و بازگشت focus را رعایت می‌کند", async () => {
    render(
      <SolverWorkspaceShell space="individual">
        <p>محتوا</p>
  ...
  it("نام انسان ثابت و نام و نقش دو تیم مستقل است", async () => {
    ...
    const first = render(
      <SolverWorkspaceShell space="team">
        <p>محتوا</p>
      </SolverWorkspaceShell>,
    );
  ...
  it("ساخت تیم، workspace، مالک، پروفایل، تنظیمات و احراز مستقل می‌سازد", () => {
    ...
    expect(state.verifications.find((item) => item.subjectId === team.id)?.documents[0].label).toBe(
      "مجوز آزمایشگاه",
    );
    expect(
      state.invitations.some(
        (item) => item.teamId === team.id && item.recipientEmail === "invitee@example.test",
      ),
    ).toBe(true);
  });
  ...
  it("کارت هر تیم و هر پیشنهاد به شناسه خودش و context کامل می‌رود", async () => {
    ...
    const hrefs = screen
      .getAllByRole("link", { name: /مدیریت تیم/ })
      .map((link) => link.getAttribute("href"));
    expect(team21).toBeInTheDocument();
    expect(
      hrefs.some((href) => href?.includes("/teams/TEAM-21") && href.includes("teamId=TEAM-21")),
    ).toBe(true);
    expect(
      hrefs.some((href) => href?.includes("/teams/TEAM-34") && href.includes("teamId=TEAM-34")),
    ).toBe(true);
    cleanup();
    ...
    expect(
      proposalLinks.some((link) => link.getAttribute("href")?.includes("/proposals/PR-104/")),
    ).toBe(true);
    expect(
      proposalLinks.some((link) => link.getAttribute("href")?.includes("/proposals/PR-127/")),
    ).toBe(true);
  ...
  it("جزئیات پیشنهاد از نسخه جاری همان رکورد ساخته می‌شود", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 1, name: "پایش پیش‌بینانه تجهیزات دوار" }),
    ).toBeInTheDocument();
  ...
  it("مشاهده پیشنهاد مستقیم accepted نمی‌سازد و فقط همان workspace را به‌روز می‌کند", async () => {
    ...
    await waitFor(() =>
      expect(readSolverState().directOffers.find((item) => item.id === "OFF-226")?.state).toBe(
        "viewed",
      ),
    );
    expect(readSolverState().directOffers.find((item) => item.id === "OFF-226")?.state).not.toBe(
      "selected",
    );
    expect(viewDirectOffer("OFF-218", PERSONAL_WORKSPACE_ID)).toMatchObject({
      ok: false,
      code: "NOT_FOUND",
    });
  ...
  it("پذیرش درخواست عضویت roster و اعلان را در یک mutation به‌روز می‌کند", () => {
    ...
    const request = before.membershipRequests.find(
      (item) => item.teamId === PRIMARY_TEAM_ID && item.state === "requested",
    )!;
  ...
  it("تنظیمات مقدار واقعی و نشست‌ها را persist می‌کند", () => {
    ...
    expect(
      saveAccountSettings({ ...current, timezone: "Europe/Berlin", smsNotifications: true }).ok,
    ).toBe(true);
  ...
  it("اعلان خوانده‌نشده واقعی و feedback پایان پرونده یک‌باره است", () => {
    ...
    const first = submitCaseFeedback(
      { type: "individual", workspaceId: PERSONAL_WORKSPACE_ID },
      "CASE-138",
      "موفق",
      "خروجی‌ها پذیرفته و تسویه شد.",
    );
    const duplicate = submitCaseFeedback(
      { type: "individual", workspaceId: PERSONAL_WORKSPACE_ID },
      "CASE-138",
      "موفق",
      "تکرار",
  ...
  it("احراز تیم Contributor فقط‌خواندنی و احراز تیم مالک قابل ارسال است", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "پرونده فقط‌خواندنی است" }),
  ...
  it("آپلود نمونه احراز validation، progress، موفقیت و remove قابل آزمون دارد", async () => {
    ...
    fireEvent.change(input, {
      target: { files: [new File(["sample"], "degree-v2.pdf", { type: "application/pdf" })] },
    });
    expect(screen.getByRole("progressbar", { name: "پیشرفت بارگذاری مدرک تخصص" })).toHaveAttribute(
      "value",
      "25",
    );
  ...
  it("Contributor در تیم دوم رابط مدیریت حساس دریافت نمی‌کند", async () => {
    at(
      `/app/solver/teams/${SECONDARY_TEAM_ID}?space=team&teamId=${SECONDARY_TEAM_ID}&workspaceId=WS-TEAM-34`,
    );
    render(<SolverTeamsOverview />);
