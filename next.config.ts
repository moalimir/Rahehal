    ...
    if (!networkRuntime) {
      config.plugins.push(
        new webpack.NormalModuleReplacementPlugin(
          /^@\/(?:components\/(?:challenge-flow\/(?:connected-record-links|live-call-controls)|internal\/network-internal-boundary|organization-preview-notice|platform-approval-queue|portal\/network-organization-login|public-challenge-(?:catalogue|record))|lib\/(?:api\/http|auth\/(?:network-organization-shell|network-session)|challenges\/adapters\/network(?:-governance|-public-challenges)?))$/,
