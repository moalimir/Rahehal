    ...
    <>
      <header className="rh-profile-heading">
        <div>
          ...
          <h1>{route.title}</h1>
          <p>{route.summary}</p>
          <small>فضای فعال: {space === "team" ? "تیمی" : "شخصی"}</small>
        </div>
      </header>
      <section className="rh-card rh-team-origin-note" role="status">
        <Icon name="history" />
        <div>
          <h2>این مقصد به جریان canonical منتقل شده است</h2>
          <p>
            برای جلوگیری از نمایش داده یا کنترل مستقل، ادامه کار را از یکی از مقصدهای وابسته به
            رکورد انجام دهید.
          </p>
        </div>
      </section>
      <section className="rh-saved-grid" aria-label="مقصدهای مرتبط">
        {destinations.map((item) => (
          <article className="rh-card rh-saved-card" key={item.href}>
            <header>
            ...
            <footer>
              <Link href={buildSolverHref(item.href, context)}>ادامه در مقصد مرتبط</Link>
            </footer>
          </article>
        ))}
      </section>
