...
| **B3**    | `eligibility_rule` table: versioned `allowed_applicant_types`, verification/NDA/document requirements, deadline, tied to the challenge version it governs.                                                                                                                                                                                                                     | `verification` | migration and PostgreSQL tests prove every existing/new challenge version receives one append-only rule snapshot in the version transaction; editable challenges reject closed, paused, or expired rules, while incomplete drafts may retain a null deadline until readiness validation                                                                                                     |
