...
**Foundation prepared:** migration `0011` establishes solver-workspace ownership, mandatory current/latest pointers, assigned-membership references, full content shape with integer minor-unit money, immutable exact challenge-version acceptance, gap-free/base-linked versions, and server-state/deadline lock guards. The canonical proposal transition table now has one shared implementation. These are prerequisites, not completed C1–C4 endpoints; milestone statuses remain unchanged until their executable API/browser acceptance passes.

| Milestone | Deliverable                                                                                                                                                                                                                                                               | Status        | Acceptance                                                                                                                                                                                                                                    |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **C1**    | Server-side `evaluateEligibility` port: ports the existing client-side eligibility logic behind the API, versioned against the B3 `eligibility_rule`, with explainable reasons/next-actions (closes FR-SOL-002).                                                          | `not-started` | the same inputs evaluate static requirements against the exact published rule, while current availability comes from the aggregate's live state/deadline; ineligible responses include a specific, non-generic reason                         |
