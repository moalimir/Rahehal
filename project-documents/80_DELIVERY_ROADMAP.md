...
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **A1a**   | PostgreSQL foundation: exact Compose pin; checksummed reversible SQL migrations; tenant/user/identity-link/workspace/membership/`access_grant`/session/challenge+version/audit/outbox/idempotency tables; deterministic synthetic seeds. | `done` | `npm run test:postgres` proves migration down/up through `0010`, seed rerun, and negative constraints (62 tests). Pinned-image startup verified 2026-08-29: both images pull from their registries and the full Compose stack reaches `healthy`. The `postgres` image is now digest-pinned like `dex` was — it had been tag-only, which is not an exact pin. |
...
| Milestone | Deliverable                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Status | Acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **B1**    | Extend the challenge state machine through `triage` → `formulation` → `approvals`, sharing one server-side readiness/validation contract across draft, preview, and submit (closes FR-ORG-003).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `done` | Invalid transitions are rejected by the state machine, and one shared readiness contract backs draft, preview and submit. All three legs are now proven: `apps/api/test/api.test.ts` asserts the 422 from `:request-triage` carries exactly the `fields` and `readiness` the draft read returned, and `tests/challenge-preview-readiness.test.tsx` asserts the preview renders the _server's_ issues and none of the browser validator's for the same record — so a drift between the two implementations fails the suite instead of reaching a user. (An earlier note here claimed the central equivalence was untested; that was wrong for the draft/submit half, which `64bc3ac` already covered.)                                                                                                                                                                                                                                                                                                                                                     |
...
| **B3**    | `eligibility_rule` table: versioned `allowed_applicant_types`, verification/NDA/document requirements, deadline, tied to the challenge version it governs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `done` | migration and PostgreSQL tests prove every existing/new challenge version receives one append-only rule snapshot in the version transaction; editable challenges reject closed, paused, or expired rules, while incomplete drafts may retain a null deadline until readiness validation. Reviewed 2026-08-28: no correctness bugs. The rule is created by a database trigger rather than an explicit adapter command (B1/B2's shape) and has no in-memory representation; the owner reviewed that divergence and accepted it as a kept one-off, not the template — later milestones (C1+, B4+) follow the B1/B2 pattern (commit ac44d70).                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **B5**    | Public discovery API serves only from `challenge_public_projection`; the private aggregate is never queried on an unauthenticated path. `GET /api/v1/public/challenges` (keyset cursor, optional `category`) and `GET /api/v1/public/challenges/{challengeId}` read a separate `PublicChallengePort`; its PostgreSQL adapter selects an explicit column list from the projection table alone. Audience is decided server-side from session presence: anonymous readers see `public` rows, a valid session additionally sees `registered`. Free-text `q` search is not delivered and lands with the discovery UI.                                                                               | `done` | A PostgreSQL test renames `challenge`/`challenge_version`/`challenge_approval` out of reach and proves both public reads still succeed, so no unauthenticated path can touch the private aggregate. The response's exact field set is snapshot-asserted in-memory and natively, and no confidential value appears in the payload. A `registered` challenge is absent from the anonymous list and returns a non-enumerating `NOT_FOUND` on detail, identical to an unknown id; unpublished and NDA challenges are equally invisible even to an authenticated org owner. A stale or malformed credential degrades to the anonymous audience. Security review completed 2026-08-29 (see §17); public-endpoint rate limiting is a gate-G item, not a B5 gap.                                                                                                                                                                                                                                                                                                  |
| **B6**    | MVP live-call controls: extend deadline, pause, resume, close, and cancel. Every command carries an explicit reason and version; server time decides deadline races.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `done` | Migration `0009` moves the call's live state (`publication_state`, `proposal_deadline_at`) onto `challenge`, leaving the approved version and its `eligibility_rule` untouched. The commands are `org:publisher`-only, idempotent, version-aware, and atomically persist receipt/audit/outbox evidence; the supplied reason is retained in `audit_event.metadata`. Migration `0010` requires any projection state/deadline update to match the authoritative aggregate, closing direct-SQL divergence. Tests prove forward-only deadlines, terminal close/cancel, pause/resume discovery behavior, role denial, rollback, synchronized projections, and supported worker events. Material amendment and `challenge:publish-override` are separate future exception paths: amendment depends on Phase-3 proposal semantics, and override remains gated by [DEC-2026-012](25_DECISIONS.md).                                                                                                                                                                 |
| **B7**    | Real-browser E2E: org drafts → triage → approvals → publish; the publisher manages the live call; public discovers and visits it; every required actor uses a visible server-backed workflow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `done` | `RAHHAL_CONNECTED_E2E=1 npm run test:browser:b7` passes 3/3 against rebuilt Docker images. Four distinct actors clear four gates through visible forms: the technical approver uses the org governance page; platform legal, finance, and ops start from the role-scoped B8 queue and open the allowlisted approval brief. `org:publisher` publishes, pauses, resumes, and extends the deadline through the UI; close/cancel controls are visibly available behind terminal-action confirmation. An anonymous context finds the record in the server-backed catalogue, opens the public projection, and sees no confidential value. Negative controls cover under-approved publish, distinct-actor enforcement, and non-enumerating unknown/unpublished reads.                                                                                                                                                                                                                                                                                            |
...
B2's standing platform authority now has the two read surfaces needed to make it usable without widening the organization aggregate:

- **B8a:** `GET /api/v1/platform/challenges/{challengeId}/approval-brief` returns a closed, allowlisted read model only while the challenge is at `approvals`. It omits contact details, invitees, attachment ids, tenant/creator ids, and other approvers' user ids; separation of duty is represented by a boolean for the current actor. The org-only `GET /api/v1/challenges/{id}` no longer has a platform fallback.
- **B8b:** `GET /api/v1/platform/challenge-approvals` lists at most 50 challenges awaiting the active platform role's own gate and excludes versions on which that actor already recorded a gate. The connected `/app/ops/publication` screen activates the platform workspace, loads that queue, and links to the scoped brief.

API, PostgreSQL, contract, and browser tests cover both surfaces. The rebuilt macOS Docker stack applies migration `0010`, passes restart-persistence smoke, and passes the A3 and B7 browser suites 3/3 each. The 2026-08-31 web rebuild additionally proves B5 catalogue navigation and B6 publisher controls against that same stack.
