...
| **B7**    | Real-browser E2E: org drafts → triage → approvals → publish; public visits the published challenge; every required actor uses a visible server-backed workflow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `done` | `RAHHAL_CONNECTED_E2E=1 npm run test:browser:b7` passes 3/3 against rebuilt Docker images. Four distinct actors clear four gates through visible forms: the technical approver uses the org governance page; platform legal, finance, and ops start from the role-scoped B8 queue and open the allowlisted approval brief. `org:publisher` publishes through the UI; an anonymous context reads the public projection with confidential values absent. Negative controls cover under-approved publish, distinct-actor enforcement, and non-enumerating unknown/unpublished reads.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| B5 public discovery | built            | connected public challenge view (`/challenges/record?id=`)          |

B7 closes the governed publication journey end to end. The fixture-backed discovery catalogue still fabricates publisher, eligibility, and match score and is therefore _not_ the surface B5 feeds; replacing that broader catalogue belongs to Phase 3's eligibility/discovery work, not Phase 2's publication gate.
...
API, PostgreSQL, contract, and browser tests cover both surfaces. The rebuilt macOS Docker stack applies migration `0010`, passes restart-persistence smoke, and passes the A3 and B7 browser suites 3/3 each.
