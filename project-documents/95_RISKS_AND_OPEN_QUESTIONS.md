...
> **▶ Recommended answers added (2026-08-23).** Every open question in §3–§9 now carries a **▶ Recommendation** — the best option for _this_ project (governed open-innovation, Persian-first, invite-only Iran pilot, cross-tenant collaboration), consistent with the canonical model and decisions **D1–D15** in [00_OVERVIEW](00_OVERVIEW.md) §4. These are engineering-ratified defaults ready to build against; items tagged _(needs … sign-off)_ still require the named business/legal/finance owner to formally accept before pilot. The §2 P0 table's "Proposed default" column already reflects these.
...
**Status update (2026-08-27), per [82_PHASE0_COMPLETION](82_PHASE0_COMPLETION.md):** this register is the original 2026-08-20 audit snapshot and is not auto-updated as work lands — treat the items below as resolved/narrowed rather than open, and everything else in the table as still accurate. **R-10 (Sharp/libvips):** resolved — `npm ci --include=optional` installs it deterministically (82 §1). **R-06 (offline bundle smoke):** resolved — the login-to-workspace flow renders correctly after the T-C harness fix (82 §3). **R-08 (Playwright/visual):** partially resolved — Chromium behavior tests are pinned, installed, and green (66/7/0 in 82 §1); only the visual Golden Master (T-E) remains open. **R-09 (no CI/CD or migrations):** partially resolved — `ci.yml` runs every required release gate and checksummed reversible migrations exist (A1a); the first clean Linux CI run and staging/production environment/rollback definitions remain open. **R-01/R-02 (browser-authority, no backend):** partially narrowed — A1a/A1b/A1c give the challenge-draft create/read/save slice real server authority ([80_DELIVERY_ROADMAP](80_DELIVERY_ROADMAP.md) Phase 1); every other stage of the lifecycle (proposal, review, decision, execution, payment) is still browser/mock authority until its own roadmap phase lands.
