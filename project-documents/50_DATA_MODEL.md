...
This section is present through migration `0013`; sections 6 onward remain later migrations. It is numbered above `0012` deliberately: the Phase-3 foundation was authored on its own branch while Phase 2's review closure was authored on `main`, and both claimed `0011`. The proposal foundation was renumbered on merge so the applied order reads in dependency order rather than showing a Phase-3 table created before a Phase-2 fix.
