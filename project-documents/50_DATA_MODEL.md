...
This section is present through migration `0013`; later sections land incrementally, with C1 solver facts in `0014` and C2 team authority in `0015`. It is numbered above `0012` deliberately: the Phase-3 foundation was authored on its own branch while Phase 2's review closure was authored on `main`, and both claimed `0011`. The proposal foundation was renumbered on merge so the applied order reads in dependency order rather than showing a Phase-3 table created before a Phase-2 fix.
