...
> **Current executable proof (2026-08-23):** `apps/api` now exercises the initial session, workspace-context, and challenge-draft subset through injected ports. Its tests cover one-time OIDC exchange semantics and mismatch denial, token expiry/rotation/revocation, active-context and membership revalidation inside a shared development unit of work, role denial, workspace-scoped non-enumerating reads, stale versions, fingerprint-bound idempotent replay, schema-constrained responses, credential-free allow/deny audit, and aggregate/audit/outbox rollback. `apps/worker` validates untrusted outbox records, isolates poison events, retries with bounds, dead-letters unsupported work, and passes `event_id` as the downstream idempotency key. Both compositions are deliberately in-memory and refuse production mode; they do **not** satisfy the production hardening gates below without managed OIDC, PostgreSQL/RLS transactions, durable audit/outbox claims and dead-letter operations, file controls, and deployment evidence.
...
- **Data classification** (D-07): `public / internal / confidential / highly_sensitive` on every field/file; classification changes are audited and role-restricted. These are the engineering defaults; Legal/Privacy+Security+Product acceptance is explicitly pending in [27_PHASE1_OWNER_APPROVALS](27_PHASE1_OWNER_APPROVALS.md).
