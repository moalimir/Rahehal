...
### D1/D5 assignment read and COI boundary

Assignment rows belong to the challenge-owning organization. Their narrow cross-tenant disclosure is restricted to the exact active reviewer membership/user attached to each row; sharing a platform workspace or holding an operations role grants no reviewer read access. SQL scopes before lookup, reuses the transaction-scoped session/membership boundary, and exposes only assignment bookkeeping, the reviewer's own declaration, and an immutable packet allowlisted to organization name and challenge title. Foreign and unknown IDs are indistinguishable and record denied-access evidence. Responses are `no-store`. Migration `0022` enforces exact immutable version bindings and creates pending COI atomically; migration `0026` permits exactly one attested declaration by the assigned active reviewer and then freezes it. Clear atomically accepts the assignment. Conflict leaves materials closed and gives Operations no override path. The material query separately requires the exact active membership/user, `accepted` state and `clear` declaration, then constructs only the technical/delivery fields from the frozen proposal version and the exact frozen rubric. It never selects solver/team identity, experience narrative, commercial terms, declarations or attachment identifiers. The connected runtime does not consult the browser COI flag and no API failure falls back to demo data.
...
Only an active `platform:ops` membership in the active platform workspace can list candidates or create, cancel, and replace assignments. The Operations projection is purpose-limited to frozen proposal tracking/version identifiers, rubric/challenge identifiers, reviewer identity/workload, due/overdue state, declaration status and cancellation evidence; it never selects proposal content or solver identity. Create locks the evaluation challenge, validates the exact frozen roster, distinct reviewer-user history, active membership, two-review capacity and server-time future due date, then commits assignment, pending COI, audit, outbox, receipt and idempotency evidence together. Cancel/replace re-lock the same evidence and permit `coi-gate` or `accepted` only while scoring has not begun; a reason is mandatory and replacement appends a new row for a different active reviewer. The separate conflict queue exposes only reviewer identity, the immutable pre-COI packet, categories, reason, declaration time and assignment version needed for that action. Reviewer reads independently rejoin the exact active membership, so membership removal denies immediately. Cancellation removes active status and material reach immediately; overdue alone does not widen or revoke access under DEC-2026-020. Connected routes use same-origin cookies, server scope, and no browser-store fallback.
