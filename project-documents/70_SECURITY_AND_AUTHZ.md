...
> **Current executable proof (2026-08-27):** A1a supplies the constrained PostgreSQL foundation and A1b supplies principal-bound session/workspace authorization with digest-only credentials, revocation, membership revalidation/locking, and atomic session evidence. A1c composes those adapters with scoped challenge create/read/save: every save appends an immutable version and atomically writes an append-only receipt, audit event, outbox event, and tenant-scoped replay result. Tests cover cross-scope non-enumeration, stale/conflicting keys, concurrent retry, forced rollback, and fresh API-runtime restart persistence. PostgreSQL startup requires migration `0003` and never falls back to demo. Managed OIDC is still absent and fails closed in PostgreSQL mode; RLS, separate database roles, rate limits, operated outbox delivery, and a networked web remain pre-pilot/later roadmap gates.
