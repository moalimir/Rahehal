...
**Private-PDF implementation checkpoint (2026-09-16):** DEC-2026-019 limits challenge PDFs to the owning organization and invited/submitted solver workspaces. Application queries scope metadata before permission checks. Organization proposal-file reads require the existing exact locked-version grant and organization record permissions; solver challenge-file reads derive from a live exact-version direct-offer grant or the active bilateral submitted-proposal grant. Withdrawal/expiry/revocation removes derived access, including previously issued download URLs. No platform-wide file bypass is introduced. Upload/scan/binding/digest/token and negative membership/grant tests are recorded in [88](88_PARTICIPATION_AND_PRIVATE_PDFS.md). Historical C3 metadata-only attachment statements below no longer describe new version writes after migration `0023`.

The local adapter uses an API-only private volume, 10 MiB limits, workspace quotas, opaque exclusive-write keys, ClamAV INSTREAM plus qpdf validation and fail-closed scan states. Scan-test doubles are confined to tests. Live local scanner/image/signature health and valid/malformed/EICAR acceptance now pass on arm64. Encrypted-at-rest storage/backup and restore, parser resource isolation, log redaction for signed query strings, retention/orphan cleanup, connected-browser certification and independent human security review remain release gates; this is not a production-security pass.

## Active MVP authorization amendment — 2026-09-16

DEC-2026-018 and [26_LEAN_MVP_SCOPE](26_LEAN_MVP_SCOPE.md) replace mandatory multi-actor publication and formal review as MVP prerequisites. **M1 executable policy:** an active `org:owner` has publication/live-call management authority over its own organization's challenges without separate publisher or platform/finance/legal approvals. It does not elevate ordinary members or confer cross-tenant access. The permission matrix and separation rules below retain the delegated/governed design; owner publication is the explicit exception. Delegated publishers still require four actual approvals; technical/legal/finance and platform approvers retain their existing permissions. No owner gate signatures are manufactured. Migration `0022` binds immutable owner attribution to the exact published version and validates active owning-workspace membership; API commands retain atomic authorization, version, idempotency, audit and outbox safeguards. See [86](86_M1_OWNER_PUBLICATION.md) for verification and the human release-review gate.
