...
The local adapter uses an API-only private volume, 10 MiB limits, workspace quotas, opaque exclusive-write keys, ClamAV INSTREAM plus qpdf validation and fail-closed scan states. Scan-test doubles are confined to tests. Live scanner/image/signature health, actual valid/invalid/malware PDF acceptance, encrypted-at-rest storage/backup and restore, parser resource isolation, log redaction for signed query strings, retention/orphan cleanup and independent human security review remain release gates; this is not a production-security pass.
