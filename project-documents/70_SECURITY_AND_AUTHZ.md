...
### D1 assignment read boundary

Assignment rows belong to the challenge-owning organization. Their narrow cross-tenant disclosure is restricted to the exact active reviewer membership/user attached to each row; sharing a platform workspace or holding an operations role grants no reviewer read access. SQL scopes before lookup, returns only assignment bookkeeping, and reuses the transaction-scoped session/membership boundary. Foreign and unknown IDs are indistinguishable and record denied-access evidence. Responses are `no-store`. D1 exposes no protected materials, COI mutations, scoring or exports; the existing browser COI flag remains demo-only and cannot authorize network access. Migration `0022` enforces immutable version bindings and creates pending COI atomically. DEC-2026-018 assigns future reviewer-assignment commands to Operations and final reasoned decisions to organization owners/members; these commands remain unavailable until their own gates land.
