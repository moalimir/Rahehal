...
### D1-D6 assignment read, COI and scoring boundary

Assignment rows belong to the challenge-owning organization. Their narrow cross-tenant disclosure is restricted to the exact active reviewer membership/user attached to each row; sharing a platform workspace or holding an operations role grants no reviewer read access. SQL scopes before lookup, reuses the transaction-scoped session/membership boundary, and exposes only assignment bookkeeping, the reviewer's own declaration, and an immutable packet allowlisted to organization name and challenge title. Foreign and unknown IDs are indistinguishable and record denied-access evidence. Responses are `no-store`. Migration `0022` enforces exact immutable version bindings and creates pending COI atomically; migration `0026` permits exactly one attested declaration by the assigned active reviewer and then freezes it. Clear atomically accepts the assignment. Conflict leaves materials closed and gives Operations no override path. Material and own-score queries separately require the exact active membership/user, `clear` COI and an active accepted-through-locked assignment. Materials construct only technical/delivery fields from the frozen proposal version and exact rubric; they never select solver/team identity, experience narrative, commercial terms, declarations or attachment identifiers. Cancellation or invalidation removes both reads immediately. The connected runtime does not consult the browser COI flag and no API failure falls back to demo data.

### D2 rubric authoring boundary

Only active `org:owner` and `org:member` actors in the challenge-owning tenant/workspace can read or append its rubric. The authoritative transaction revalidates session, active context, membership and capability before scope-first challenge lookup, and performs idempotent replay inside that same unit of work. A command cites the exact published challenge version and current rubric version; assignment existence freezes the rubric before any reviewer can be bound to drifting criteria. Domain validation and migration `0023` independently enforce the 0-5/100% policy. Rubric rows and versions reject update/delete, audit/outbox contain identifiers and version numbers rather than labels, and connected browser errors never fall back to fixture or `localStorage` data.

### D3 evaluation-opening boundary

Only an active `org:owner` or `org:member` in the challenge-owning workspace can preview or open evaluation. The preview is deliberately narrow: exact accessible proposal/version IDs, tracking code, source state, accessible counts, rubric/challenge version IDs, and readiness blockers; solver identity and proposal content are absent. An inaccessible or invalid relevant proposal produces one generic `proposal_roster_unavailable` blocker and is excluded from returned counts and rows, so expired access cannot enumerate a protected record. The command scopes and locks the challenge before replay, uses transaction time, locks relevant proposals, and refuses open, paused-before-deadline, cancelled, unresolved, stale, invalid-version, or inaccessible rosters. At snapshot time every qualifying proposal must still have the active exact C4 grant to the organization. The immutable `evaluation_proposal` row then becomes the evaluation-purpose version binding, so later expiry cannot erase evidence or silently select a newer version. Migration `0024` repeats these rules in triggers, freezes later proposal/rubric versions, validates the final stage at commit, and refuses evidence-destroying rollback. Audit/outbox metadata carries version IDs and counts, without proposal content, solver identity, or rubric labels.

### D4 reviewer-assignment boundary

Only an active `platform:ops` membership in the active platform workspace can list candidates or create, cancel, and replace assignments. The Operations projection is purpose-limited to frozen proposal tracking/version identifiers, rubric/challenge identifiers, reviewer identity/workload, due/overdue state, declaration status, cancellation evidence, and D6 review status/timestamps/reasons/weighted total; it never selects proposal content, solver identity, criterion scores or rationales. Create locks the evaluation challenge, validates the exact frozen roster, distinct reviewer-user history, active membership, two-review capacity and server-time future due date, then commits assignment, pending COI, audit, outbox, receipt and idempotency evidence together. Cancel/replace re-lock the same evidence and permit unfinished `coi-gate`, `accepted`, or `draft` assignments; replacement also accepts terminal `invalidated` evidence. A reason is mandatory and replacement appends a new row for a different active reviewer absent from proposal history. The separate conflict queue exposes only reviewer identity, the immutable pre-COI packet, categories, reason, declaration time and assignment version needed for that action. Reviewer reads independently rejoin the exact active membership, so membership removal denies immediately. Cancellation removes active status and material reach immediately; overdue alone does not widen or revoke access under DEC-2026-020/021. Connected routes use same-origin cookies, server scope, and no browser-store fallback.

### D6 review evidence boundary

The reviewer draft and submit commands scope through the exact assigned active platform-reviewer membership, active assignment and clear declaration before resolving the target. Both require assignment `expected_version`, tenant-bound idempotency and one transaction. Draft validation accepts only criteria in the assigned immutable rubric and never computes a final result from incomplete evidence. Submission requires every criterion, integer 0-5 values and nonblank rationales, calculates integer-tenths weight server-side, and freezes scores before transitioning the assignment. Operations lock/invalidation separately revalidate active platform Operations authority, lock the evaluation challenge and assignment, require a reason, and commit scorecard/assignment versions plus audit/outbox/receipt/replay evidence atomically. The database independently rejects edits to submitted scores, reviewer self-lock/invalidation, non-Operations attribution, and organization decision-role invalidation. Invalidation preserves the locked scorecard and removes it from active completeness; replacement appends evidence. Outbox metadata excludes score values, rationales and reasons. D8 must lock and recheck the two independent, locked, non-invalidated reviews and must make invalidation unavailable once a decision exists.
