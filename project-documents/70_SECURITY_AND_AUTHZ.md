...
> **Current executable proof (2026-08-28):** A1a supplies the constrained PostgreSQL foundation and A1b supplies principal-bound session/workspace authorization with digest-only credentials, revocation, membership revalidation/locking, and atomic session evidence. A1c composes those adapters with scoped challenge create/read/save and atomic immutable evidence. A2 adds provider-neutral authorization-code + S256 PKCE using pinned `openid-client` and a synthetic local Dex provider: exact redirect allowlisting, server-bound state/verifier/nonce digests, signature/issuer/audience/nonce validation, verified matching contact, one-time transaction-scoped consumption, opaque credential issuance, rotation, revocation, and expiry are executable. Tests cover protocol/PKCE denial, unverified contact, replay, rollback, cross-scope non-enumeration, stale/conflicting keys, concurrent retry, forced rollback, and fresh API-runtime persistence. PostgreSQL startup requires migration `0004` and never falls back to demo. The managed production IdP, MFA/step-up, RLS, separate database roles, rate limits, operated outbox delivery, and a networked web remain pre-pilot/later roadmap gates.
