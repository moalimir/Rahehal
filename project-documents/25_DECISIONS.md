...
**Decision.** Platform-role standing authority is a second instance of that same category: a fixed, named, narrow set of platform-owned actions (currently: recording one of the four B2 publication gates, and the B8a read of a challenge that is awaiting approvals) that a `platform:*` role may perform against any org's workspace, resolved directly from an _active_ platform membership (`WorkspaceAuthorityUnitOfWorkPort.runAuthorizedPlatformRole`) rather than from an `access_grant` row. It carries the same liveness guarantee as every other membership check, and by the same mechanism: like `runAuthorizedWorkspace`, it revalidates the session and the platform membership _inside_ the same unit of work that performs the write, so revoking a session or suspending the membership denies immediately rather than racing an in-flight command. Resolving platform access through a bare lookup outside that unit of work is the one implementation shape this decision forbids — and every decision (allow and deny) is recorded through the same `audit_event` pipeline as every other authorization decision ([70_SECURITY_AND_AUTHZ](70_SECURITY_AND_AUTHZ.md) §2). It does not weaken `access_grant` for actual org↔solver collaboration, and it is not a general cross-tenant read/write escape hatch: each action a platform role reaches this way must be individually named in code (`gateApproverRoles`), never a wildcard. **Amended 2026-08-30 (B8a):** the set gains `challenge:read`, scoped to challenges in the `approvals` stage and to the platform roles derived from `gateApproverRoles`. The write without the read was not a narrower grant — it was an unusable one: a gate recorded against a document the approver cannot open is a rubber stamp, and the quality gate exists precisely so publication is not self-certified. Outside the approvals window the read answers the same non-enumerating `NOT_FOUND` an unrelated tenant gets, so this does not become a cross-tenant browse.
...
- **Status:** proposed / **pending owner decision** · **Owner:** Product + Security · **Blocking milestone:** B6 ([80](80_DELIVERY_ROADMAP.md) §5); does not block B4/B5/B7
...
- **Consequences either way:** the override needs step-up, a structured reason, its own audit action distinct from `challenge.published`, and — if joint — a two-party command with its own concurrency and expiry semantics, which is materially more work than a single reasoned command. Deciding before B6 starts avoids designing the wrong shape.
...
- **Status:** accepted (eng) / **pending owner sign-off** · **Owner:** Frontend + Product · **Blocking milestone:** B7 ([80](80_DELIVERY_ROADMAP.md) §5)
