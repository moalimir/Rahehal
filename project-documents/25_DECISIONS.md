...
### DEC-2026-018 — MVP evaluation completeness and decision authority

- **Status:** accepted 2026-09-07 (explicit owner answers) · **Owner:** Product · **Blocking milestones:** Phase 4 D2-D9
- **Decision:** require two independent human reviews per eligible proposal. Before selection, every assigned review must be locked or formally replaced, and each eligible proposal must have its two valid independent reviews. Operations assigns reviewers; organization owners and members record the final reasoned decision. The MVP supports one selected proposal and one correlated case, or an explicit reasoned no-award outcome. Shortlisting is separate from final selection.
- **Consequences:** count distinct reviewer users, not memberships or sessions. Replacement preserves the original assignment/review evidence and cannot bypass completeness. Reconcile canonical transition role lists with `org:owner` and `org:member` before activating decision commands. This acceptance does not settle rubric arithmetic, pre-COI disclosure, identity-release timing, post-decision invalidation, or step-up proof; those remain the explicit gates in roadmap section 7.

### DEC-2026-019 — MVP rubric scoring policy

- **Status:** accepted 2026-09-07 (explicit owner answer) · **Owner:** Product · **Blocking milestones:** Phase 4 D2/D6
...
### DEC-2026-021 — Loose MVP review completion operations

- **Status:** accepted 2026-09-09 (explicit owner direction) · **Owner:** Product · **Blocking milestone:** Phase 4 D6
- **Decision:** Operations may reason-cancel or replace an unfinished assignment in `coi-gate`, `accepted`, or `draft`; cancelling a draft preserves its scorecard as incomplete evidence. A submitted review is structurally complete and immutable. Operations must explicitly lock it with a reason before it counts toward evaluation completeness. A locked review may be invalidated only through a reasoned Operations command whose actor is neither its reviewer nor any active organization owner/member for that challenge. Invalidation preserves the submitted and locked evidence and requires a new independent reviewer assignment; the replacement cannot reuse any reviewer already present in that proposal's assignment history.
- **Consequences:** `cancelled` and `invalidated` assignments never count toward the two valid reviews required by DEC-2026-018. Replacement appends a new assignment against the same frozen proposal and rubric versions and never rewrites the old row or scorecard. A later decision transaction must lock and recheck this completeness set. Once a decision exists, review invalidation is unavailable unless a future accepted policy adds an explicit, authorized reopen/correction event that preserves both review and decision evidence.

### DEC-2026-022 — Loose MVP blind comparison release

- **Status:** accepted 2026-09-10 (owner's loose-MVP direction) · **Owner:** Product · **Blocking milestones:** Phase 4 D7-D9
- **Decision:** organization owners and members receive one identity-free comparison projection for the frozen evaluation roster. Completeness counts and cancelled/invalidated history counts are visible throughout evaluation. Score release is global: aggregate results remain absent for every proposal until every frozen proposal has exactly two distinct locked, non-invalidated reviews. The released result contains only the mean weighted score and mean score per rubric criterion. Reviewer identities, individual votes, rationales and operational reasons remain absent. Solver identity remains at the existing proposal tracking code and exact version baseline; no workspace label, profile or legal identity is added for the local MVP.
- **Consequences:** the projection query, rather than browser rendering, enforces the release point and revalidates stored totals against the exact rubric. Early completion of one proposal cannot influence organization handling of unfinished proposals. Operations retains its separate administrative identity/status projection; solvers and reviewers receive no D7 comparison route. An empty roster is vacuously complete and can proceed only to reasoned no-award. Any future reviewer-identity or solver-profile reveal, per-review release, or post-decision disclosure needs an explicit policy change. D8 must repeat the exact full-roster completeness test while holding the challenge lock.

### DEC-2026-023 — Local decision step-up and case-access lifetime

- **Status:** proposed; implemented for local remediation, pending explicit Product + Security acceptance (2026-09-13) · **Owner:** Product + Security · **Blocking milestones:** Phase 4 D8-D9 release
- **Approval evidence:** the owner requested implementation fixes and practical verification, but no explicit acceptance of the five-minute freshness or case-lifetime policy was recorded. This entry must not be treated as an accepted ADR. See [86_PHASE4_REVIEW_AND_BROWSER_AUDIT](86_PHASE4_REVIEW_AND_BROWSER_AUDIT.md).
- **Proposed decision / current local behavior:** the local MVP reuses the configured OIDC provider for forced reauthentication. A successful callback issues one opaque proof valid for at most five minutes and bound to the exact user, app-session version, tenant, workspace, `challenge.decision.record` action, and challenge. Finalization consumes it once in the same transaction as the decision; missing, expired, consumed, wrong-session, wrong-workspace, wrong-actor, wrong-action, and wrong-target proofs return `STEP_UP_REQUIRED`. The browser callback may return only to the exact allowlisted evaluation route and must preserve an in-progress decision form without persisting its sensitive content to browser storage.
- **Case access:** selection creates an active `collaborate` grant for the selected solver workspace with PostgreSQL `infinity` as its time bound. This represents the case relationship's lifetime, not permanent irrevocable access: an authorized case-state or access-revocation command must explicitly move the grant to a terminal state when the relationship ends. A fixed 365-day expiry is rejected because it silently removes an active party from a still-open case. No-award creates no case and no case grant.
- **Consequences:** local step-up is provider-bound evidence, not a claim that managed production MFA exists; ADR-0010 and pre-pilot G1 still require the managed IdP/MFA policy. Stable local Dex lacks `auth_time`; an explicitly enabled non-production, insecure-loopback-only adapter uses signed token `iat` for the local fixture. Token issuance time is not independent proof of user reauthentication. Production refuses that setting and retains strict `auth_time` verification. Start/completion allow/deny outcomes and the final command remain correlated audit evidence. The proposed freshness/lifetime choices need explicit acceptance before release; any later change also requires an accepted policy.
