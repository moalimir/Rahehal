...
- **Local-first container baseline:** the root multi-stage `Dockerfile` and `compose.yaml` build Linux images for the current static web, compiled API, and compiled worker on macOS Docker Desktop or Linux. The local stack binds only to `127.0.0.1`, runs non-root/read-only with dropped capabilities, and carries explicit demo-mode configuration. It proves portable packaging and process health only; the API/worker remain isolated in-memory compositions and the stack is not a staging or production environment.
...
- **Session** = short-lived access token + rotating refresh; server-side revocation list; membership changes and session revocation **immediately** deny protected operations (a pre-pilot hardening gate).
