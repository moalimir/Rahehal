...
## Active MVP policy amendment — 2026-09-16

DEC-2026-018 in [25_DECISIONS](25_DECISIONS.md) and [26_LEAN_MVP_SCOPE](26_LEAN_MVP_SCOPE.md) amend the active product policy. The canonical entity names, 11-stage vocabulary and role namespaces remain; the stage table below records the existing governed model, not an instruction to require every stage or actor in the lean MVP.

- **Publication (M1 implementation):** `org:owner` can author/manage and publish its own organization's complete challenge from `draft`, `triage`, `formulation` or `approvals`, without mandatory platform triage, a separate publisher, or independent technical/legal/finance approvals. Publication checks required content, deadline, version, active membership and owning workspace; locks exact evidence; snapshots eligibility; creates only the allowlisted public projection; and records receipt/audit/outbox atomically. Owner publication is attributed separately, never represented by fabricated gate decisions. `org:publisher` retains the existing four-gate `approvals → published` path; all other delegated roles remain unchanged. Owner edits in unpublished triage/approvals create a new formulation version, preserving earlier locked evidence. Owners and publishers can manage the live call; published content remains immutable.
- **Decision target (M2, not implemented):** an organization decision cites the exact immutable submitted proposal/response and a reason, without mandatory rubric, reviewer assignment or locked review evidence. Replace the old `reviews-complete` prerequisite for this path through an explicit shared-domain/API/database change. Organization selection followed by explicit acceptance from an authorized actor in the selected solver workspace confirms the match, without routine platform approval. Bind acceptance to an exact shared agreement summary and evidence version; selection alone is not agreement. No-award, rejection, decline, withdrawal and cancellation require durable outcomes. Section 5 of [26](26_LEAN_MVP_SCOPE.md) lists the remaining role/timing/state mapping policies; the accepted actors and required exit paths are no longer open questions.
- Do not invent a second lifecycle, silently auto-approve intermediate states, or claim the new transition graph exists today. M1/M2 must document their exact allowed edges and guards with implementation. `selected`, mutual agreement, and contract effectiveness must not be conflated.
- Case, Review, Contract, Pilot and Payment remain canonical future entities; their presence here does not make their full workflow part of this MVP. Formal reviewer COI/access and payment separation rules still apply whenever those deferred capabilities are enabled.
