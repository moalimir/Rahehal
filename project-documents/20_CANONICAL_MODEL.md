...
- **Publication target (M1, not implemented):** `org:owner` can author/manage and publish its own organization's challenge without mandatory platform triage, a separate publisher, or independent technical/legal/finance approvals. Publication still checks required content, version, membership and owning workspace; locks exact evidence; creates the public projection; and records receipt/audit/outbox atomically. Existing delegated publication must not be accidentally removed.
