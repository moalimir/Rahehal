...
| **Review Assignment** | The record connecting a reviewer to a proposal version + rubric version + due date.                                                                                       | implied by `ReviewState` (`packages/domain/src/review.ts`)                         |
...
- **Assignment lifecycle** (`ReviewState`): `coi-gate → accepted → draft → submitted → locked → invalidated`; Operations may move a pre-scoring `coi-gate` or `accepted` assignment to terminal `cancelled` with a reason and append a distinct replacement.
