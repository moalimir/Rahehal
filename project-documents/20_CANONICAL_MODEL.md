...
**D8-D9 delivered boundary (DEC-2026-023):** organization owners/members may append shortlist versions after the full frozen roster is complete. A selected decision must cite the latest shortlist and an exact shortlisted proposal version; no-award may omit a shortlist, including for an empty roster. Finalization repeats completeness while holding the challenge lock, requires per-proposal feedback and a structured outcome-compatible reason, and consumes one fresh local OIDC step-up proof bound to the exact actor, session version, tenant/workspace, decision action, and challenge. The proof expires after at most five minutes and cannot be replayed. One transaction appends the immutable decision and exact review-evidence links, records every proposal outcome, moves the challenge to `decided`, consumes the proof, and commits receipt/audit/outbox/idempotency evidence. Selection additionally creates exactly one `case_record` and an active, revocable case-lifetime solver collaboration grant; no-award creates neither. Organization and the selected solver workspace can read the case through their own current authority, while each solver reads only its own outcome and feedback. Managed production MFA remains a separate pre-pilot gate.

**Local verification qualification (2026-09-13):** DEC-2026-023 describes implemented remediation, not an accepted product/security policy. Its five-minute freshness and case-lifetime choices require explicit acceptance before release. The stable local Dex fixture uses an opt-in signed-token-issuance-time compatibility adapter; that is not independent proof of fresh user authentication or production MFA. See [86_PHASE4_REVIEW_AND_BROWSER_AUDIT](86_PHASE4_REVIEW_AND_BROWSER_AUDIT.md) for the browser/database evidence and remaining gates.
