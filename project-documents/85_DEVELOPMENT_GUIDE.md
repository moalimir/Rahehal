...
`docker:build` refreshes changed targets. `docker:up` waits for Dex and PostgreSQL, runs the guarded one-shot migration/seed container, then starts the PostgreSQL-composed API and waits for HTTP health. `docker:smoke` creates a challenge with the synthetic session, restarts the API container, and reads the same record back. Dex exposes synthetic local logins, including `owner-alpha@synthetic.invalid` and D1 reviewers `reviewer-alpha@synthetic.invalid` / `reviewer-beta@synthetic.invalid` (local password `rahhal-local-owner`) and an exact `http://localhost:3000/auth/browser/callback` redirect, which A3 connects to the web through the same-origin browser-session routes. C7's local email/mobile verification provider uses the configured synthetic code `12345`; it keeps challenges in process memory, persists no OTP payload, and refuses production startup. The images run as unprivileged users on read-only root filesystems with all Linux capabilities dropped; writable temporary space is bounded `tmpfs`. Published ports bind to `127.0.0.1`, not the LAN. Do not weaken those defaults to simulate a server.

The stack is intentionally incomplete but no longer browser-authoritative for the delivered connected slices. Phases 1-3 supply PostgreSQL identity/workspace authorization, challenge/proposal/team authority, local OIDC and contact activation, durable in-app notification projection, and connected organization/solver journeys. Phase 4 D1-D9 add reviewer/Operations/organization review flows, local provider-bound decision step-up, atomic decision/case continuity, and connected solver outcomes. D10 integration/notifications and D11 certification remain incomplete. The managed production IdP/MFA, RLS, platform-wide abuse controls, private file pipeline, external delivery, and independently operated audit remain later gates; the local PostgreSQL outbox worker is authoritative only for its implemented in-app projection.
...
D8-D9 add decision routes through migrations `0028` and `0029`. A shortlist is optional for no-award and mandatory/latest for selection. Starting finalization opens local OIDC with `prompt=login` and `max_age=0`; success returns to the exact evaluation route, keeps the unsaved decision form in memory, and issues an HttpOnly proof for at most five minutes. The proof is single-use and exact-context bound. Selection creates one case plus an active case-lifetime solver grant; no-award creates neither. DEC-2026-023's freshness/lifetime choices are proposed and need explicit Product + Security acceptance before release. Stable Dex does not emit `auth_time`; Compose explicitly opts into `OIDC_ALLOW_LOCAL_ISSUED_AT_STEP_UP` for non-production insecure-loopback fixtures only. Signed `iat` proves token issuance, not fresh user authentication. Production rejects this compatibility setting and remains strict; managed MFA is not implied.
...
whose rollback you intend to test. For a complete local rebuild, use `npm run db:reset`; it drops and
recreates the local schema, applies all migrations up, seeds deterministic data, and refuses non-loopback targets.
...
The Phase-4 suite creates unique governed empty-roster and eligible-proposal fixtures in loopback PostgreSQL. It runs owner rubric/open-evaluation UI, Operations assignment, two independent reviewer COI/materials/draft/submit sessions, Operations lock, owner comparison/shortlist/decision with popup OIDC, and solver outcome/case reads. The empty-roster scenario separately proves reasoned no-award and immutable evidence:

```bash
npm run docker:up
npm run build:web:network
RAHHAL_CONNECTED_E2E=1 npm run test:browser:phase4
```

The npm command runs desktop only. For all seven configured viewports, run `RAHHAL_CONNECTED_E2E=1 PLAYWRIGHT_SKIP_WEBSERVER=1 node --env-file-if-exists=.env node_modules/playwright/cli.js test playwright/phase4.pw.ts --workers=1 --output=output/playwright/phase4-final`. The suite refuses a non-loopback `DATABASE_URL`. Each project seeds its own activated synthetic human with a permanent individual workspace and explicit team membership, preserving identity-link uniqueness and the provider's three-start-per-contact abuse limit. It retains immutable fixtures. Do not restart services or run `docker:smoke` during the browser run; run persistence smoke first. The 2026-09-13 confirmation passes 14/14, but the broader D10/D11 negative, notification, identity and human-review gates in [86_PHASE4_REVIEW_AND_BROWSER_AUDIT](86_PHASE4_REVIEW_AND_BROWSER_AUDIT.md) still apply. A timeout in login/setup is a harness failure, not decision-flow acceptance.
