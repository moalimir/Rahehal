...
The combined Phase-2/Phase-3-foundation gate passes 52 Vitest files / 388 tests, 6 PostgreSQL files / 74 tests, 66 applicable demo-browser checks (4 viewport-specific skips), all eight performance budgets, and the route/link/offline/standalone checks. The static JavaScript artifact is 1,814,805 bytes against the accepted engineering ceiling of 1,819,000. The current Phase-2 connected image also passes A3 and B7 at 3/3 each against PostgreSQL after migration `0011_c_proposal_foundation`; `docker:smoke` confirms API-restart persistence without resetting the existing volume.
