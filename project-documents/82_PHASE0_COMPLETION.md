...
Phase 3 is still at its prepared foundation, not a completed delivery milestone. Migration `0013` now creates the solver-owned proposal/version boundary with exact accepted challenge-version evidence, immutable locked versions, current/latest pointer consistency, supported ISO-4217 currencies (`IRR`, `USD`, `EUR`), and server-state/deadline guards. The revised C1–C10 plan now follows amended DEC-2026-004 and DEC-2026-016: a development-provider OTP scheme supports self-service human solver activation; optional team onboarding creates a separate, initially unverified workspace; the exact challenge rule conditionally gates verification at submission; authoritative summaries/notifications support the bilateral flow; and the route-level [84_PHASE3_CONNECTED_MVP_AUDIT](84_PHASE3_CONNECTED_MVP_AUDIT.md) ends in full frontend synchronization plus separate real-browser certification. All C milestones remain `not-started` until their API, transaction, provider, migration, browser, and negative-authorization acceptance tests land.
