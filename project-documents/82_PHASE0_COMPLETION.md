...
Phase 3 has begun but has not completed its delivery gate. Migration `0013` creates the solver-owned proposal/version foundation. C1 is now built and in `verification`: migration `0014`, typed profile/verification/gate contracts, server routes, demo/PostgreSQL adapters, and the exact-rule/live-state eligibility evaluator are present, with native unit/API coverage. Its focused PostgreSQL suite is committed as evidence but remains unexecuted in this local milestone because Docker/database testing was explicitly excluded, so C1 is not `done`. C2–C10 remain `not-started`; they continue to follow amended DEC-2026-004 and DEC-2026-016 for development-provider activation, optional separate team onboarding, conditional verification at submission, bilateral notifications, frontend synchronization, and real-browser certification.
