...
The final local gate passes 57 Vitest files / 401 tests, 5 PostgreSQL files / 67 tests, 66 applicable demo-browser checks (4 viewport-specific skips), both connected browser suites at 3/3, all eight performance budgets, and the route/link/offline/standalone checks. The static JavaScript artifact is 1,818,758 bytes against the accepted engineering ceiling of 1,819,000; connected-only session/routing, governance, and public-projection components are excluded from that demo bundle rather than raising the budget.
