...
Phase 3 is still at its prepared foundation, not a completed delivery milestone. Migration `0011` now creates the solver-owned proposal/version boundary with exact accepted challenge-version evidence, immutable locked versions, current/latest pointer consistency, supported ISO-4217 currencies (`IRR`, `USD`, `EUR`), and server-state/deadline guards. C1–C7 remain `not-started` until their API, transaction, browser, and negative-authorization acceptance tests land.
