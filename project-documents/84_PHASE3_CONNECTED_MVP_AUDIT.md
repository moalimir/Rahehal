...
6. After authentication, public headers, mobile drawers, footers, auth routes, and workspace entry points show the signed-in identity and relevant workspace actions instead of continuing to advertise signup/login.
...
3. **Session chrome:** after login, desktop header, mobile drawer, footer, `/auth/*`, and `/app` consistently expose identity/workspace actions and no signup/login prompts; sign-out clears connected state and protected routes return to session-required.
