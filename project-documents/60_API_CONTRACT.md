...
- **Step-up**: sensitive commands (`sensitiveActions`, `product.ts:42`: decide, accept-deliverable, approve-payment, manage-access, resolve-dispute) require a fresh `step_up_token`; absence or an expired/consumed/wrong-context proof → `403` with `code:"STEP_UP_REQUIRED"`. D8's local decision proof is valid for at most five minutes, single-use, and bound to actor, app-session version, tenant/workspace, action, and target challenge.
...
The OpenAPI now has exactly **106 paths / 115 operations**: the completed Phase-1/2 and Phase-3 surface plus Phase-4 D1-D9 reviewer, rubric, evaluation, assignment, COI, scoring, blind-comparison, shortlist, decision, outcome, and case operations. The same-origin browser decision step-up start/callback remain browser-session adapters rather than bearer-token OpenAPI operations. Every implemented write carries `expected_version` and `Idempotency-Key`; every protected solver/team/profile/proposal/review route and protected challenge write additionally requires `X-Workspace-Id`. B3's challenge create/save fields (`verification_required`, `document_gate_required`, `allowed_applicant_types`, `nda_required`, and `proposal_deadline`) become the immutable rule snapshot C1/C4 read for the exact published version. Entries labelled future below are not part of the current contract. PostgreSQL mode validates signed issuer/audience/nonce, exact state/redirect, S256 PKCE, the existing `(issuer, subject)` link, and a verified matching contact before issuing digest-only app credentials. C7 adds an independent provider assertion route for self-service solvers; contact verification remains distinct from workspace verification. RLS, the managed production IdP, and managed production MFA remain later gates.
...
**D4-D6 implemented Operations commands:** `GET/POST /api/v1/operations/review-assignments` and `POST /api/v1/operations/review-assignments/{assignmentId}:cancel|:replace|:lock|:invalidate` require active `platform:ops` authority. The list returns frozen evaluation slots, proposal tracking/version and rubric IDs, active counts, reviewer candidates/workload, assignment/cancellation bookkeeping, and an administrative review summary containing status timestamps, lock/invalidation reasons and the weighted total; it omits proposal content, solver identity, criterion scores and rationales. Create requires the current evaluation/challenge `expected_version`, active reviewer membership, frozen proposal ID and future due date. Cancel/replace require the assignment `expected_version` and reason; replacement also requires a different active reviewer and future due date. They admit unfinished `coi-gate`, `accepted`, or `draft` rows; an invalidated row can only be replaced. Every path preserves the old packet/declaration/scorecard, caps active assignments at two, rejects reviewer-user history reuse, and commits metadata-only audit/outbox/receipt/idempotency evidence atomically.

**D5 implemented COI and materials:** `POST /api/v1/assignments/{assignmentId}/coi:declare` requires the exact active assigned reviewer, `Idempotency-Key`, `expected_version`, explicit attestation, and either a clear declaration with no relationship/reason or a conflict with one or more validated categories and a nonblank reason. It atomically freezes the declaration, increments the assignment version, accepts a clear assignment or preserves a conflicted one, and writes audit/outbox/receipt/replay evidence. There is no conflict override or separate accept bypass. `GET /api/v1/assignments/{assignmentId}/materials` returns `NOT_FOUND` unless the exact membership remains active, the assignment is `accepted`, `draft`, `submitted`, or `locked`, and its declaration is `clear`; success returns the frozen proposal/rubric version IDs, exact rubric criteria, and an explicit technical/delivery proposal projection. Solver/team identity, history, commercial terms, declarations and attachment IDs are structurally absent. Cancellation or invalidation removes this reach. `GET /api/v1/operations/review-conflicts` gives active Operations only the reviewer, pre-COI packet, categories, reason, time and assignment version needed to cancel/replace; it contains no proposal identity/content or solver identity. All reads are `no-store`.

**D6 implemented scoring and evidence lifecycle:** `GET /api/v1/assignments/{assignmentId}/review` returns `null` before the first draft and otherwise only the exact active reviewer's own scorecard. `POST /api/v1/assignments/{assignmentId}/review:save-draft` accepts exact-shape partial scores while the clear assignment is `accepted` or `draft`; `POST /api/v1/assignments/{assignmentId}/review:submit` accepts only a saved complete draft, calculates the weighted total server-side and freezes score evidence. Both require the current assignment `expected_version` and `Idempotency-Key`. Operations `:lock` and `:invalidate` commands require reason, optimistic concurrency and idempotency; the actor must differ from the reviewer, and invalidation additionally rejects a current owner/member of that challenge organization. Submission, lock and invalidation record separate attribution/timestamps. Audit may retain the operational reason and total; outbox payloads contain only IDs and versions, never scores, rationales or reasons.

**D7 implemented blind comparison:** `GET /api/v1/challenges/{challengeId}/review-comparison` requires an active organization owner/member in the challenge-owning workspace and returns `NOT_FOUND` outside that scope or before an evaluation snapshot exists. It always returns the exact challenge/proposal/rubric versions, tracking codes, rubric criteria, full-roster completion, and active/locked/cancelled/invalidated counts. `score_summary` is `null` on every proposal until the entire roster has two distinct locked, non-invalidated reviews per proposal. After that global release point it contains only integer-tenths weighted and per-criterion means. Reviewer identities, memberships, individual reviews/votes, rationales, operational reasons, solver/workspace identities, proposal content and commercial terms are structurally absent. The server query and domain calculation enforce release; all reads are `no-store` and the demo adapter returns `STORAGE` rather than fixtures.

**D8-D9 implemented:** `GET /api/v1/challenges/{challengeId}/decision` returns organization-scoped readiness, exact frozen proposal/rubric/version references, latest shortlist, final decision, feedback outcomes and optional case. `POST /api/v1/challenges/{challengeId}/decision-shortlist-versions` appends a reasoned exact-version shortlist only after the full review roster is complete. `POST /auth/browser/challenges/{challengeId}/decision-step-up:start` begins forced local OIDC reauthentication for the exact current session/workspace/challenge and returns only the provider URL/expiry; its callback places the proof in an HttpOnly same-origin cookie and returns only to the allowlisted evaluation route. `POST /api/v1/challenges/{challengeId}/decision:record` requires organization owner/member authority, the current aggregate/evaluation versions, structured outcome-compatible reason, exact feedback for the full roster, latest shortlist for selection, idempotency, and a fresh exact-context proof. It atomically captures the counting reviews, records every proposal outcome, moves the challenge to `decided`, consumes the proof, and creates exactly one selected case/grant or no case for no-award.

`GET /api/v1/proposals/{proposalId}/outcome` begins from the active solver workspace and reveals only that proposal's status, exact version, feedback, decision time and optional case ID. `GET /api/v1/cases/{caseId}` begins independently from either the owning organization workspace or the selected solver's active case grant; a failure to read the case does not erase a successfully read proposal outcome. Protected foreign/unknown records remain non-enumerating and reads are `no-store`.

**2026-09-13 local verification qualification:** the five-minute proof and case-lifetime policy in DEC-2026-023 remain proposed pending explicit Product + Security acceptance. Stable local Dex omits `auth_time`; the explicitly enabled non-production HTTP loopback compatibility adapter uses signed `iat`, which proves token issuance time, not independently fresh user authentication. Production rejects this adapter setting and retains the strict freshness boundary. The connected multi-actor browser suite passes 14/14 across seven viewports; callback-outage error/log sanitization and D10/D11 closure remain open in [86_PHASE4_REVIEW_AND_BROWSER_AUDIT](86_PHASE4_REVIEW_AND_BROWSER_AUDIT.md).

## 6. Read projections & anonymity

- Organization review views honor DEC-2026-022 in the projection query: completeness/history counts are visible during evaluation, every score is absent until the full frozen roster is complete, and released values are aggregate-only. Reviewer and solver identities never enter the D7 response.
